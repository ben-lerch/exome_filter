...
double minLRT;
vector <double> LRT;
vector <string> LRTpred;


string AFfile;
string Annofile;
string GERPfile;
string phastConsfile;
string phyloPfile;
string CADDfile;
string MutationTasterScorefile;
string MutationAssessorScorefile;
string LRTfile;
...
cout << "reading pedigree, bro" << endl;
...
void applyFilters(string myVCFgeno, vector <string> inVCF,  vector <int> DISEASEinVCF, vector <int> SEQUENCEinVCF, int k, double myMAF){
...
if(filters[i]=="-novelin1000G"){ if( (af[k] - myMAF) < 0.005 ){filterPass++;}else{break;}}
...
int pass = 1; 
for(int i=0; i<inVCF.size(); i++){ if( (SEQUENCEinVCF[i]==1 && DISEASEinVCF[i]==0 && myVCFgeno.substr(i*2,2)!="00" ) || (SEQUENCEinVCF[i]==1 && DISEASEinVCF[i]==1 && myVCFgeno.substr(i*2,2)!="01") ){pass=0; break;}} 
if(pass==1){filterPass++;} 
}

if(filters[i]=="-SIFT"){ if( SIFT[k] < maxSIFT ){filterPass++; Rinki++;}}

if(filters[i]=="-PolyPhen2"){ if( PolyPhen2[k] > minPolyPhen2 ){filterPass++; Rinki++;}}

if(filters[i]=="-LRTpred"){ if( LRTpred[k] != "N"){filterPass++; Rinki++;}}

if(filters[i]=="-MutationTasterPred"){if(MutationTasterPred[k]!="N" && MutationTasterPred[k]!="P"){filterPass++;Rinki++;}}

if(filters[i]=="-MutationAssessorPred"){if(MutationAssessorPred[k]=="H" || MutationAssessorPred[k]=="M" || MutationAssessorPred[k]=="."){filterPass++;Rinki++;}}
